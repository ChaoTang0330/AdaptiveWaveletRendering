...
void render(const std::string& sceneFilePath)
{
    ...
    Wavelet* wl = new Wavelet("coefs.txt", scene->imageSize.x);
    ...
    TimePoint startTime = Clock::now();
    {
        ...
        while (numCompletedSamples < sampleBudget)
        {
            ...
            while (numCompletedJobs < jobs.size()) 
            {
                ...
                numCompletedJobs += completedJobs.size();
                printLoadingBar(static_cast<double>(numCompletedSamples) / sampleBudget);
                //cout << "\r Completed samples, " << numCompletedSamples;
            }
            delete pool;
        }

        wl->denoise(imageData, scene->gamma);//, finalImage
    }
    ...
    //    //gamma correction
    //    finalImage[i].r = pow((double)avgColor.r, 1 / (double)scene->gamma);
    //    finalImage[i].g = pow((double)avgColor.g, 1 / (double)scene->gamma);
    //    finalImage[i].b = pow((double)avgColor.b, 1 / (double)scene->gamma);
    //}
